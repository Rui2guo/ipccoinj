...
public class LegacyAddress extends Address {
    ...
    public static LegacyAddress fromBase58(@Nullable NetworkParameters params, String base58)
        ...
        int version[] = new int[3];
         version[0] = versionAndDataBytes[0] & 0xFF;
         version[1] = versionAndDataBytes[1] & 0xFF;
         version[2] = versionAndDataBytes[2] & 0xFF;
        byte[] bytes = Arrays.copyOfRange(versionAndDataBytes, 3, versionAndDataBytes.length);
        if (params == null) {
            ...
            if (Arrays.equals(version,params.getP2SHHeader()))
                return new LegacyAddress(params, true, bytes);
            else if (Arrays.equals(version,params.getAddressHeader()))
                return new LegacyAddress(params, false, bytes);
//            if (Arrays.equals(version,params.getAddressHeader()))
//                return new LegacyAddress(params, false, bytes);
//            else if (Arrays.equals(version,params.getP2SHHeader()))
//                return new LegacyAddress(params, true, bytes);
