...
public class Base58 {
    ...
    public static String encodeChecked(int[] version, byte[] payload) {
       ...
       /* String hexStr = DatatypeConverter.printHexBinary(payload);
        payload  = DatatypeConverter.parseHexBinary(""+hexStr);*/
        byte[] addressBytes = new byte[3+payload.length + 4];
        addressBytes[0] = (byte) version[0];
        addressBytes[1] = (byte) version[1];
        addressBytes[2] = (byte) version[2];
        System.arraycopy(payload, 0, addressBytes, 3, payload.length);
        byte[] checksum = Sha256Hash.hashTwice(addressBytes, 0, payload.length+3);
        System.arraycopy(checksum, 0, addressBytes, payload.length+3 , 4);
