...
import org.IPCcoinj.core.listeners.*;
import org.IPCcoinj.core.Address;
import org.IPCcoinj.core.AbstractBlockChain;
import org.IPCcoinj.core.BlockChain;
import org.IPCcoinj.core.BloomFilter;
import org.IPCcoinj.core.Coin;
import org.IPCcoinj.core.Context;
import org.IPCcoinj.core.ECKey;
import org.IPCcoinj.core.FilteredBlock;
import org.IPCcoinj.core.InsufficientMoneyException;
import org.IPCcoinj.core.LegacyAddress;
import org.IPCcoinj.core.Message;
import org.IPCcoinj.core.NetworkParameters;
import org.IPCcoinj.core.Peer;
import org.IPCcoinj.core.PeerFilterProvider;
import org.IPCcoinj.core.PeerGroup;
import org.IPCcoinj.core.Sha256Hash;
import org.IPCcoinj.core.StoredBlock;
import org.IPCcoinj.core.Transaction;
import org.IPCcoinj.core.TransactionBag;
import org.IPCcoinj.core.TransactionBroadcast;
import org.IPCcoinj.core.TransactionBroadcaster;
import org.IPCcoinj.core.TransactionConfidence;
import org.IPCcoinj.core.TransactionInput;
import org.IPCcoinj.core.TransactionOutPoint;
import org.IPCcoinj.core.TransactionOutput;
import org.IPCcoinj.core.UTXO;
import org.IPCcoinj.core.UTXOProvider;
import org.IPCcoinj.core.UTXOProviderException;
import org.IPCcoinj.core.Utils;
import org.IPCcoinj.core.VerificationException;
import org.IPCcoinj.core.TransactionConfidence.*;
import org.IPCcoinj.crypto.*;
import org.IPCcoinj.script.*;
import org.IPCcoinj.signers.*;
import org.IPCcoinj.utils.*;
import org.IPCcoinj.wallet.Protos.Wallet.*;
import org.IPCcoinj.wallet.WalletTransaction.*;
import org.IPCcoinj.wallet.listeners.KeyChainEventListener;
import org.IPCcoinj.wallet.listeners.ScriptsChangeEventListener;
import org.IPCcoinj.wallet.listeners.WalletChangeEventListener;
import org.IPCcoinj.wallet.listeners.WalletCoinsReceivedEventListener;
import org.IPCcoinj.wallet.listeners.WalletCoinsSentEventListener;
import org.IPCcoinj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
...
public class Wallet extends BaseTaggableObject
     ...
     * @throws org.IPCcoinj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
     ...
     */
    public Transaction createSend(Address address, Coin value, int type) throws InsufficientMoneyException {
        SendRequest req = SendRequest.to(address, value, type);
        if (params.getId().equals(NetworkParameters.ID_UNITTESTNET))
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public SendResult sendCoins(TransactionBroadcaster broadcaster, Address to, Coin value, int type) throws InsufficientMoneyException {
        SendRequest request = SendRequest.to(to, value,type);
        ...
        try {
            ...
            for (UTXO output : getStoredOutputsFromUTXOProvider()) {
                ...
                if (!excludeImmatureCoinbases || !coinbase || depth >= params.getSpendableCoinbaseDepth()) {
                    candidates.add(new FreeStandingTransactionOutput(params, output, chainHeight, type));
                }
            }
    ...
    private class FreeStandingTransactionOutput extends TransactionOutput {
         ...
         */
        public FreeStandingTransactionOutput(NetworkParameters params, UTXO output, int chainHeight,int type) {
            super(params, null, output.getValue(), output.getScript().getProgram(), type);
            this.output = output;
    ...
    private FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        while (true) {
            ...
            if (change.isGreaterThan(Coin.ZERO)) {
                ...
                TransactionOutput changeOutput = new TransactionOutput(params, tx, change, changeAddress,type);
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.IPCcoinj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.IPCcoinj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private List<Transaction> maybeRotateKeys(@Nullable KeyParameter aesKey, boolean sign) throws DeterministicUpgradeRequiresPassword {
        ...
        Transaction tx;
        do {
            tx = rekeyOneBatch(keyRotationTimestamp, aesKey, results, sign, type);
    ...
    @Nullable
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign, int type) {
        lock.lock();
        try {
            ...
            rekeyTx.addOutput(toMove.valueGathered, sign ? freshReceiveAddress() : currentReceiveAddress(),type);
